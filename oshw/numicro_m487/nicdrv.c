...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
   ...
   rVal = NU_EMAC_init((uint8_t *)priMAC);
   if (rVal != 0)
      return 0;
...
//   mtx_lock (port->getindex_mutex);
	 xSemaphoreTake( port->getindex_mutex, 0xffffffff);
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   rval = NU_EMAC_send((*stack->txbuf)[idx], lp);
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   if (port->redstate != ECT_RED_NONE)
   {
      ...
      xSemaphoreTake( port->tx_mutex, 0xffffffff);
      ...
      //ASSERT (0);
      NU_EMAC_send(&(port->txbuf2), port->txbuflength2);
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   bytesrx = NU_EMAC_recv((*stack->tempbuf), lp);
...
int ecx_inframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   {
			xSemaphoreTake( port->rx_mutex, 0xffffffff);
...
int ecx_srconfirm(ecx_portt *port, int idx, int timeout)
{
   int wkc = EC_NOFRAME;
   osal_timert timer;
	 //printf("[%s %d]Got timeout=%d after BRD command.\r\n", __func__, __LINE__,  timeout );
