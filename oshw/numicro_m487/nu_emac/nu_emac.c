...
//Wayne: Use polling RX pool, the IRQ routing is not neccessary.
...
void dump_buf(const char* caller, uint8_t* pu8Buf, int length)
{
	int i=0;
	return ;
	printf("\n\n================================\n" );		
	printf("[%s-%s] %p %d\n", caller, __func__, pu8Buf, length);		
	printf("\n--------------------------------\n" );		
	for(i=0;i<16;i++)
		printf("%02X ", pu8Buf[i]);
	printf("\n");			
...
// Receive a packet from RX pool, free buffer and re-trigger RXDMA.
...
#endif
            //ethernetif_input(status & 0xFFFF, cur_rx_desc_ptr->buf, cur_rx_desc_ptr->status2, (u32_t)cur_rx_desc_ptr->next);
						if ( (status & 0xFFFF) <= size )
							memcpy(  (void*)packet, (void*)cur_rx_desc_ptr->buf, (status & 0xFFFF) );
						else
							printf("[Warning] packet buffer size is too small. %d > %d \r\n", (status & 0xFFFF), size );

						ret = (status & 0xFFFF);
						dump_buf(__func__, cur_rx_desc_ptr->buf, (status & 0xFFFF));


        }
			  
				//printf("[%s %d] %d %d\r\n", __func__, __LINE__, (status & 0xFFFF), size );
...
// Send a packet to TX pool and trigger TXDMA.
int NU_EMAC_send(void *packet, int length)
{
		u8_t * pbuf= NULL;
		//Request TX buffer to transmit packet.
		if ( (pbuf=ETH_get_tx_buf())==NULL )
			return -1;

		memcpy(pbuf, packet, length);

		dump_buf(__func__, pbuf, length );
		ETH_trigger_tx(length, NULL);
		return 0;
}
