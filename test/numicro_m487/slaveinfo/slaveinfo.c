...
#define false 0
#define true 1
static void test_osal_timer_timeout_us (const uint32_t timeout_us)
{
   osal_timert timer;
	 ec_timet ec_timet =osal_current_time();
	
//	 printf("[%s]timeout-%d, (%d.%d)\r\n", __func__, timeout_us, ec_timet.sec, ec_timet.usec );

   while(!(timeout_us > 4000));

   osal_timer_start (&timer, timeout_us); 	//3, 
	 if(osal_timer_is_expired (&timer) == true) while(1);

 	 osal_usleep (timeout_us - 2000);					//11, 
   if(osal_timer_is_expired (&timer) == true) while(1);

   osal_usleep (4000);											//15
   if(osal_timer_is_expired (&timer) == false) while(1);	
}
 ...
 */
static void test_osal_current_time_for_delay_us (const int32_t sleep_time_us)
{
   ec_timet start;
   ec_timet stop;
   int32_t measurement_us;
   int32_t deviation_us;
   const int32_t usleep_accuracy_us = USECS_PER_TICK;
   ...
   while (!is_deviation_within_tolerance);
}
